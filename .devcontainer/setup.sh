...
echo "[⚠️] Очистка окружения Codespace..."

# === Удаление предыдущего контейнера и docker-compose файлов ===
sudo docker-compose -f ~/dockercom/ubuntu_gui.yml down --remove-orphans
sudo docker rm -f ubuntu_gui 2>/dev/null
sudo docker image rm -f dorowu/ubuntu-desktop-lxde-vnc:bionic 2>/dev/null

# === Очистка директории проекта ===
rm -rf ~/dockercom

# === Очистка dangling образов и мусора ===
sudo docker system prune -af --volumes

echo "[✅] Очищено. Начинаем установку заново..."

# === 1. Установка Docker и необходимых пакетов ===
echo "[+] Обновление пакетов и установка Docker и Docker Compose..."
sudo apt update && sudo apt install -y docker.io docker-compose openvpn curl unzip resolvconf
